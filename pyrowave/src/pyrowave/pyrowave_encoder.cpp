...
// Portable (C++20 <bit>) replacements for the GCC/Clang builtins. std::countl_zero
// and std::countr_zero already return 32 for a zero argument, matching the guards.
#define popcount32_(x) (std::popcount(static_cast<uint32_t>(x)))
#define leading_zeroes_(x) (std::countl_zero(static_cast<uint32_t>(x)))
#define trailing_zeroes_(x) (std::countr_zero(static_cast<uint32_t>(x)))
...
bool Encoder::encode(vk::raii::CommandBuffer & cmd, const ViewBuffers & views, const BitstreamBuffers & buffers)
{
	...
	if (wavelet_img_low_res)
	{
		...
		image_barrier.image = vk::Image(static_cast<VkImage>(wavelet_img_low_res));
		cmd.pipelineBarrier(
		        vk::PipelineStageFlagBits::eComputeShader,
		        vk::PipelineStageFlagBits::eComputeShader,
		        {},
