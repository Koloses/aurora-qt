...
class Encoder : public WaveletBuffers
{
	...
	// NB: struct named pipeline_t (not pipeline) so its `pipeline` member does not
	// collide with the injected-class-name; MSVC otherwise resolves `x.pipeline`
	// to the type rather than the member.
	struct pipeline_t
	{
	...
	pipeline_t block_packing_;
	pipeline_t resolve_rdo_;
	pipeline_t analyze_rdo_;
	pipeline_t analyze_rdo_finalize; // shares descriptor set with anaylze_rdo
